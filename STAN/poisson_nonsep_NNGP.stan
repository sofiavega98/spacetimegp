/**
 * Poisson Non-Separable NNGP (Nearest Neighbor Gaussian Process) Model
 * 
...
functions {
  ...
  real st_cov_nonsep(real h, real u, real sigma2, real beta, real alpha,
    ...
    real denom = a * pow(fabs(u), 2*alpha) + 1;  // Temporal dependence
...
parameters {
  ...
  real<lower=0.1,upper=10> sigma_global;    // Overall std dev
