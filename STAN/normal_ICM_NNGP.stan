/**
 * Normal ICM-NNGP (Intrinsic Coregionalization Model with Nearest Neighbor Gaussian Process)
 * 
 ...
 * The model is designed for analyzing continuous spatiotemporal data with treatment effects.
 * 
 * Model Structure:
...
data {
  ...
  int<lower=1> num_treated;           // Number of treated observations
  int<lower=1> control_idx[N*D-num_treated]; // Control observation indices
...
model {
  ...
  y[control_idx] ~ normal(intercept + f[control_idx], sigma_obs);
}

generated quantities {
  vector[num_treated] f_samples;    // Posterior samples for treated units
  ...
  for (i in 1:num_treated) {
    f_samples[i] = normal_rng(intercept + f[i], sigma_obs);
  }
